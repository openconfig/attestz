...
func TestEncryptWithSymmetricKeySuccess(t *testing.T) {
	u := &DefaultTPM12Utils{}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			ciphertext, err := u.EncryptWithAES(symKey, tc.data)
			if err != nil {
			...
			if len(ciphertext) < blockSize {
				t.Fatalf("Ciphertext too short to contain IV: %d", len(ciphertext))
			}
			iv := ciphertext[:blockSize]
			encrypted := ciphertext[blockSize:]
...
func TestEncryptWithSymmetricKeyFailure(t *testing.T) {
	u := &DefaultTPM12Utils{}
	symKey, err := u.NewAESCBCKey(tpm12.AlgAES128)
	if err != nil {
		t.Fatalf("NewAESCBCKey() failed: %v", err)
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			_, err := u.EncryptWithAES(symKey, tc.data)
			if err == nil {
				t.Errorf("EncryptWithSymmetricKey(%v) expected an error, got nil", tc.data)
			}
		})
	}
}
